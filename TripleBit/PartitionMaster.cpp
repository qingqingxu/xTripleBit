...
PartitionMaster::PartitionMaster(TripleBitRepository*& repo, const ID parID) {
	...
	for (int type = 0; type < 2; type++) {
		...
		for (chunkID = 1; chunkID < xChunkNumber[type]; chunkID++) {
			xChunkQueue[type][chunkID] = new TasksQueueChunk(startPtr + chunkID * MemoryBuffer::pagesize - sizeof(ChunkManagerMeta), chunkID, type);
...
void PartitionMaster::endupdate() {
	for (int soType = 0; soType < 2; ++soType) {
		...
		if (!xChunkTempBuffer[soType][chunkID]->isEmpty()) {
			combineTempBufferToSource(xChunkTempBuffer[soType][chunkID], startPtr, chunkID, soType);
		}
		for (chunkID = 1; chunkID < xChunkNumber[soType]; ++chunkID) {
			if (!xChunkTempBuffer[soType][chunkID]->isEmpty()) {
				combineTempBufferToSource(xChunkTempBuffer[soType][chunkID], startPtr - sizeof(ChunkManagerMeta) + chunkID * MemoryBuffer::pagesize, chunkID, soType);
			}
		}
	}
}
...
void PartitionMaster::executeInsertData(SubTrans* subTransaction) {
	...
	/*ofstream s("searchChunkIDByS", ios::app);
	 ...
	 << ",object, " << object;*/
	chunkID = partitionChunkManager[ORDERBYS]->getChunkIndex()->searchChunk(subjectID, object);
	/*s << ",chunkID, " << chunkID << endl;
	 s.close();*/
	shared_ptr<IndexForTT> indexForTT(new IndexForTT(2));
	ChunkTask *chunkTask1 = new ChunkTask(subTransaction->operationType, subjectID, object, objType, subTransaction->triple.scanOperation, taskPackage, indexForTT);
	...
	/*ofstream o("searchChunkIDByO", ios::app);
	 ...
	 << ",subjectID, " << subjectID;*/
	chunkID = partitionChunkManager[ORDERBYO]->getChunkIndex()->searchChunk(object, subjectID);
	/*o << ",chunkID, " << chunkID << endl;
	 o.close();*/
	ChunkTask *chunkTask2 = new ChunkTask(subTransaction->operationType, subjectID, object, objType, subTransaction->triple.scanOperation, taskPackage, indexForTT);
...
void PartitionMaster::executeDeleteClause(SubTrans* subTransaction) {
	...
	if (!subTransaction->triple.constSubject && !subTransaction->triple.constObject) {
		chunkIDMax = partitionChunkManager[ORDERBYS]->getChunkIndex()->getTableSize() - 1;
		chunkCount = chunkIDMax - chunkIDMin + 1;
		size_t chunkCountO = 0, chunkIDMinO = 0, chunkIDMaxO = 0;
		chunkIDMaxO = partitionChunkManager[ORDERBYO]->getChunkIndex()->getTableSize() - 1;
		chunkCountO = chunkIDMaxO - chunkIDMinO + 1;
		shared_ptr<IndexForTT> indexForTT(new IndexForTT(chunkCount + chunkCountO));
		shared_ptr<SubTaskPackageForDelete> taskPackage(new SubTaskPackageForDelete(chunkCount, subTransaction->operationType, subjectID, subTransaction->triple.constSubject, subTransaction->triple.constObject));
		if (chunkCount != 0) {
			for (size_t offsetID = chunkIDMin; offsetID <= chunkIDMax; offsetID++) {
				ChunkTask *chunkTask = new ChunkTask(subTransaction->operationType, subjectID, object, objType, subTransaction->triple.scanOperation, taskPackage, indexForTT);
				taskEnQueue(chunkTask, xChunkQueue[ORDERBYS][offsetID]);
			}
		}
		shared_ptr<SubTaskPackageForDelete> taskPackageO(new SubTaskPackageForDelete(chunkCountO, subTransaction->operationType, object, objType, subTransaction->triple.constSubject, subTransaction->triple.constObject));
		if (chunkCountO != 0) {
			for (size_t offsetID = chunkIDMinO; offsetID <= chunkIDMaxO; offsetID++) {
				ChunkTask *chunkTask = new ChunkTask(subTransaction->operationType, subjectID, object, objType, subTransaction->triple.scanOperation, taskPackageO, indexForTT);
		...
		if (partitionChunkManager[ORDERBYS]->getChunkIndex()->searchChunk(subjectID, subjectID + 1, chunkIDMin)) {
			chunkIDMax = partitionChunkManager[ORDERBYS]->getChunkIndex()->searchChunk(subjectID, UINT_MAX);
	...
	} else if (!subTransaction->triple.constSubject && subTransaction->triple.constObject) {
		if (partitionChunkManager[ORDERBYO]->getChunkIndex()->searchChunk(object, object + 1, chunkIDMin)) {
			chunkIDMax = partitionChunkManager[ORDERBYO]->getChunkIndex()->searchChunk(object, UINT_MAX);
			assert(chunkIDMax >= chunkIDMin);
			chunkCount = chunkIDMax - chunkIDMin + 1;
		} else {
			return;
		...
		if (chunkCount != 0) {
			shared_ptr<SubTaskPackageForDelete> taskPackage(new SubTaskPackageForDelete(chunkCount, subTransaction->operationType, object, objType, subTransaction->triple.constSubject, subTransaction->triple.constObject));
			shared_ptr<IndexForTT> indexForTT(new IndexForTT(chunkCount));
			for (size_t offsetID = chunkIDMin; offsetID <= chunkIDMax; offsetID++) {
				ChunkTask *chunkTask = new ChunkTask(subTransaction->operationType, subjectID, object, objType, subTransaction->triple.scanOperation, taskPackage, indexForTT);
				taskEnQueue(chunkTask, xChunkQueue[ORDERBYO][offsetID]);
...
void PartitionMaster::executeUpdate(SubTrans *subTransfirst, SubTrans *subTranssecond) {
	...
	cout << __FUNCTION__ << endl;
#endif
	ID subjectID = subTransfirst->triple.constSubject ? subTransfirst->triple.subjectID : 0;
	double object = subTransfirst->triple.constObject ? subTransfirst->triple.object : 0;
	char objType = subTransfirst->triple.constObject ? subTransfirst->triple.objType : NONE;
	ID subUpdate = subTranssecond->triple.constSubject ? subTranssecond->triple.subjectID : 0;
	double obUpdate = subTranssecond->triple.constObject ? subTranssecond->triple.object : 0;
	char objUpdate = subTranssecond->triple.constObject ? subTranssecond->triple.objType : NONE;
	size_t xChunkIDMin = 0, xChunkIDMax = 0;
	size_t chunkCount = 0;
	if (subTransfirst->triple.constSubject) { //update object
		if (partitionChunkManager[ORDERBYS]->getChunkIndex()->searchChunk(subjectID, subjectID + 1, xChunkIDMin)) {
			xChunkIDMax = partitionChunkManager[ORDERBYS]->getChunkIndex()->searchChunk(subjectID, UINT_MAX);
			assert(xChunkIDMax >= xChunkIDMin);
		...
		shared_ptr<IndexForTT> indexForTT(new IndexForTT(chunkCount));
		shared_ptr<SubTaskPackageForDelete> taskPackage(new SubTaskPackageForDelete(chunkCount, subTransfirst->operationType, subjectID, subTranssecond->triple.constSubject, subTranssecond->triple.constObject));
		for (size_t offsetID = xChunkIDMin; offsetID <= xChunkIDMax; offsetID++) {
			ChunkTask *chunkTask = new ChunkTask(subTransfirst->operationType, subjectID, object, objType, subUpdate, obUpdate, objUpdate, subTransfirst->triple.scanOperation, taskPackage, indexForTT);
			taskEnQueue(chunkTask, xChunkQueue[ORDERBYS][offsetID]);
		}
		indexForTT->wait();
	} else { //update subject
		if (partitionChunkManager[ORDERBYO]->getChunkIndex()->searchChunk(object, object + 1, xChunkIDMin)) {
			xChunkIDMax = partitionChunkManager[ORDERBYO]->getChunkIndex()->searchChunk(object, UINT_MAX);
			assert(xChunkIDMax >= xChunkIDMin);
			chunkCount = xChunkIDMax - xChunkIDMin + 1;
		} else {
			return;
		...
		shared_ptr<IndexForTT> indexForTT(new IndexForTT(chunkCount));
		shared_ptr<SubTaskPackageForDelete> taskPackage(new SubTaskPackageForDelete(chunkCount, subTransfirst->operationType, object, objType, subTransfirst->triple.constSubject, subTransfirst->triple.constObject));
		for (size_t offsetID = xChunkIDMin; offsetID <= xChunkIDMax; offsetID++) {
			ChunkTask *chunkTask = new ChunkTask(subTransfirst->operationType, subjectID, object, objType, subUpdate, obUpdate, objUpdate, subTransfirst->triple.scanOperation, taskPackage, indexForTT);
			taskEnQueue(chunkTask, xChunkQueue[ORDERBYO][offsetID]);
		}
...
void PartitionMaster::executeChunkTaskInsertData(ChunkTask *chunkTask, const ID chunkID, const uchar *startPtr, const bool soType) {
#ifdef MYDEBUG
	cout << __FUNCTION__ << endl;
#endif
	xChunkTempBuffer[soType][chunkID]->insertTriple(chunkTask->Triple.subjectID, chunkTask->Triple.object, chunkTask->Triple.objType);
...
void PartitionMaster::readIDInTempPage(const uchar *&currentPtrTemp, const uchar *&endPtrTemp, const uchar *&startPtrTemp, char *&tempPage, char *&tempPage2, bool &theOtherPageEmpty, bool &isInTempPage) {
...
void PartitionMaster::handleEndofChunk(const uchar *startPtr, uchar *&chunkBegin, uchar*&startPtrChunk, uchar *&currentPtrChunk, uchar *&endPtrChunk, const uchar *&startPtrTemp, char *&tempPage, char *&tempPage2, bool &isInTempPage, bool &theOtherPageEmpty, double min, double max, bool soType, const ID chunkID) {
...
void PartitionMaster::combineTempBufferToSource(TempBuffer *buffer, const uchar *startPtr, const ID chunkID, const bool soType) {
#ifdef MYDEBUG
	...
	if (chunkID == 0) {
		...
		startPtrChunk = currentPtrChunk = chunkBegin + sizeof(ChunkManagerMeta) + sizeof(MetaData);
	...
	endPtrTemp = startPtrTemp + ((MetaData*) startPtrTemp)->usedSpace;
	ChunkTriple *tempTriple, *bufferTriple = buffer->getBuffer(), *endTempBuffer = buffer->getEnd();
	...
	if (currentPtrTemp >= endPtrTemp) {
		...
		currentPtrTemp = partitionChunkManager[soType]->readXY(currentPtrTemp, tempTriple->subjectID, tempTriple->object, tempTriple->objType);
	}
	while (lastPtrTemp < endPtrTemp && bufferTriple < endTempBuffer) {
		//the Ptr not reach the end
		if (tempTriple->subjectID == 0 || (tempTriple->subjectID == bufferTriple->subjectID && tempTriple->object == bufferTriple->object && tempTriple->objType == bufferTriple->objType)) {
			tmp << tempTriple->subjectID << "," << partitionID << "," << tempTriple->object << endl;
			...
			if ((soType == ORDERBYS && (tempTriple->subjectID < bufferTriple->subjectID || (tempTriple->subjectID == bufferTriple->subjectID && tempTriple->object < bufferTriple->object) || (tempTriple->subjectID == bufferTriple->subjectID && tempTriple->object == bufferTriple->object && tempTriple->objType < bufferTriple->objType))) || (soType == ORDERBYO && (tempTriple->object < bufferTriple->object || (tempTriple->object == bufferTriple->object && tempTriple->subjectID < bufferTriple->subjectID) || (tempTriple->object == bufferTriple->object && tempTriple->subjectID == bufferTriple->subjectID && tempTriple->objType < bufferTriple->objType)))) {
				uint len = currentPtrTemp - lastPtrTemp;
				if (currentPtrChunk + len > endPtrChunk) {
					handleEndofChunk(startPtr, chunkBegin, startPtrChunk, currentPtrChunk, endPtrChunk, startPtrTemp, tempPage, tempPage2, isInTempPage, theOtherPageEmpty, min, max, soType, chunkID);
				}
				...
				max = getChunkMinOrMax(tempTriple, soType) > max ? getChunkMinOrMax(tempTriple, soType) : max;
				currentPtrChunk += len;
				tmp << tempTriple->subjectID << "," << partitionID << "," << tempTriple->object << endl;
				ch << tempTriple->subjectID << "," << partitionID << "," << tempTriple->object << endl;
				...
				uint len = sizeof(ID) + sizeof(char) + Chunk::getLen(bufferTriple->objType);
				if (currentPtrChunk + len > endPtrChunk) {
					handleEndofChunk(startPtr, chunkBegin, startPtrChunk, currentPtrChunk, endPtrChunk, startPtrTemp, tempPage, tempPage2, isInTempPage, theOtherPageEmpty, min, max, soType, chunkID);
				}
				if (currentPtrChunk == startPtrChunk) {
				...
				partitionChunkManager[soType]->writeXY(currentPtrChunk, bufferTriple->subjectID, bufferTriple->object, bufferTriple->objType);
				max = getChunkMinOrMax(bufferTriple, soType) > max ? getChunkMinOrMax(bufferTriple, soType) : max;
				currentPtrChunk += len;
				tb << bufferTriple->subjectID << "," << partitionID << "," << bufferTriple->object << endl;
				ch << bufferTriple->subjectID << "," << partitionID << "," << bufferTriple->object << endl;
				bufferTriple++;
			}
		}
	}
...
void PartitionMaster::executeChunkTaskDeleteData(ChunkTask *chunkTask, const ID chunkID, const uchar* startPtr, const bool soType) {
#ifdef MYDEBUG
	cout << __FUNCTION__ << endl;
#endif
	...
	if (soType == ORDERBYS) {
		while (reader < limit) {
			...
			if (tempSubjectID < subjectID || (tempSubjectID == subjectID && tempObject < object) || (tempSubjectID == subjectID && tempObject == object && tempObjType < objType)) {
				continue;
			} else if (tempSubjectID == subjectID && tempObject == object && tempObjType == objType) {
				/*ofstream finds("finds", ios::app);
				 finds << "chunkID," << chunkID << "," << subjectID << ","
				 << partitionID << "," << object << endl;
				 finds.close();*/
				temp = partitionChunkManager[soType]->deleteTriple(temp, objType);
		...
		if (subjectID == 0) { //删除所有记录，主要用于基于模式删除
			while (reader < limit) {
				reader = (const uchar*) partitionChunkManager[soType]->deleteTriple(const_cast<uchar*>(reader));
				partitionChunkManager[soType]->tripleCountDecrease();
			}
			while (metaData->NextPageNo) {
				chunkBegin = reinterpret_cast<uchar*>(TempMMapBuffer::getInstance().getAddress()) + MemoryBuffer::pagesize * metaData->NextPageNo;
			...
			while (reader < limit) {
				temp = const_cast<uchar*>(reader);
				reader = partitionChunkManager[soType]->readXY(reader, tempSubjectID, tempObject, tempObjType);
				if (tempObject < object || (tempObject == object && tempSubjectID < subjectID) || (tempObject == object && tempSubjectID == subjectID && tempObjType < objType)) {
					continue;
				} else if (tempObject == object && tempObjType == objType && tempSubjectID == subjectID) {
...
void PartitionMaster::deleteDataForDeleteClause(MidResultBuffer *buffer, const bool soType, const bool constSubject, const ID subjectID, const double object, const char objType) {
	/*
	 #ifdef MYDEBUG
	 cout << __FUNCTION__ << endl;
	 #endif
	...
	if (soType == ORDERBYS) {
		if (constSubject) { //subject是常量，仅删除对应的object
			...
			for (size_t i = 0; i < buffer->getUsedSize(); ++i) {
				chunkID = partitionChunkManager[ORDERBYO]->getChunkIndex()->searchChunk(objects[i].object, subjectID);
				ChunkTask *chunkTask = new ChunkTask(operationType, subjectID, objects[i].object, objects[i].objType, scanType, taskPackage, indexForTT);
				taskEnQueue(chunkTask, xChunkQueue[ORDERBYO][chunkID]);
			}
			indexForTT->wait();
		} else { //subject是未知量，删除所有subject与object
			size_t chunkCount = 0, chunkIDMin = 0, chunkIDMax = 0;
			chunkIDMax = partitionChunkManager[ORDERBYO]->getChunkIndex()->getTableSize() - 1;
			chunkCount = chunkIDMax - chunkIDMin + 1;
			shared_ptr<SubTaskPackageForDelete> taskPackage(new SubTaskPackageForDelete());
			if (chunkCount != 0) {
				shared_ptr<IndexForTT> indexForTT(new IndexForTT(chunkCount));
				for (size_t offsetID = chunkIDMin; offsetID <= chunkIDMax; offsetID++) {
					ChunkTask *chunkTask = new ChunkTask(operationType, 0, object, objType, scanType, taskPackage, indexForTT); //subjectID为0表示删除所有记录
					taskEnQueue(chunkTask, xChunkQueue[ORDERBYO][offsetID]);
				}
				indexForTT->wait();
			}
		...
		shared_ptr<IndexForTT> indexForTT(new IndexForTT(buffer->getUsedSize()));
		for (size_t i = 0; i < buffer->getUsedSize(); ++i) {
			chunkID = partitionChunkManager[ORDERBYS]->getChunkIndex()->searchChunk(subejctIDs[i], object);
			ChunkTask *chunkTask = new ChunkTask(operationType, subejctIDs[i], object, objType, scanType, taskPackage, indexForTT);
...
void PartitionMaster::executeChunkTaskDeleteClause(ChunkTask *chunkTask, const ID chunkID, const uchar *startPtr, const bool soType) {
	/*
	 #ifdef MYDEBUG
	 cout << __FUNCTION__ << endl;
	 #endif
		...
		if (soType == ORDERBYS) {
			...
			if (tempSubjectID < subjectID) {
				...
				if (chunkTask->taskPackageForDelete->constObject) {
					if (tempObject < object || (tempObject == object && tempObjType < objType)) {
						continue;
...
void PartitionMaster::updateDataForUpdate(MidResultBuffer *buffer, const ID subjectID, const double object, const char objType, const ID updateSubjectID, const double updateObject, const char updateObjType, const bool soType) {
#ifdef MYDEBUG
	cout << __FUNCTION__ << endl;
#endif
	int chunkID;
	...
	if (soType == ORDERBYS) {
		MidResultBuffer::SignalO* objects = buffer->getObjectBuffer();
		shared_ptr<IndexForTT> indexForTTForDelete(new IndexForTT(buffer->getUsedSize())); //删除副本o
		for (size_t i = 0; i < buffer->getUsedSize(); ++i) {
			chunkID = partitionChunkManager[ORDERBYO]->getChunkIndex()->searchChunk(objects[i].object, subjectID);
			ChunkTask *delOChunkTask = new ChunkTask(opDelete, subjectID, objects[i].object, objects[i].objType, scanType, taskPackage, indexForTTForDelete);
			taskEnQueue(delOChunkTask, xChunkQueue[ORDERBYO][chunkID]);
		}
		indexForTTForDelete->wait();
		shared_ptr<IndexForTT> indexForTTForInsert(new IndexForTT(2)); //插入副本s，o
		chunkID = partitionChunkManager[ORDERBYS]->getChunkIndex()->searchChunk(subjectID, updateObject);
		ChunkTask *insertSChunkTask = new ChunkTask(opInsert, subjectID, updateObject, updateObjType, scanType, taskPackage, indexForTTForInsert);
		taskEnQueue(insertSChunkTask, xChunkQueue[ORDERBYS][chunkID]);
		chunkID = partitionChunkManager[ORDERBYO]->getChunkIndex()->searchChunk(updateObject, subjectID);
		ChunkTask *insertOChunkTask = new ChunkTask(opInsert, subjectID, updateObject, updateObjType, scanType, taskPackage, indexForTTForInsert);
		taskEnQueue(insertOChunkTask, xChunkQueue[ORDERBYO][chunkID]);
		indexForTTForInsert->wait();
	} else if (soType == ORDERBYO) {
		ID* subejctIDs = buffer->getSignalIDBuffer();
		shared_ptr<IndexForTT> indexForTTForDelete(new IndexForTT(buffer->getUsedSize())); //删除副本s
		for (size_t i = 0; i < buffer->getUsedSize(); ++i) {
			chunkID = partitionChunkManager[ORDERBYS]->getChunkIndex()->searchChunk(subejctIDs[i], object);
			ChunkTask *delSChunkTask = new ChunkTask(opDelete, subejctIDs[i], object, objType, scanType, taskPackage, indexForTTForDelete);
			taskEnQueue(delSChunkTask, xChunkQueue[ORDERBYS][chunkID]);
		}
		indexForTTForDelete->wait();
		shared_ptr<IndexForTT> indexForTTForInsert(new IndexForTT(2)); //插入副本s，o
		chunkID = partitionChunkManager[ORDERBYS]->getChunkIndex()->searchChunk(updateSubjectID, object);
		ChunkTask *insertSChunkTask = new ChunkTask(opInsert, updateSubjectID, object, objType, scanType, taskPackage, indexForTTForInsert);
		taskEnQueue(insertSChunkTask, xChunkQueue[ORDERBYS][chunkID]);
		chunkID = partitionChunkManager[ORDERBYO]->getChunkIndex()->searchChunk(object, updateSubjectID);
		ChunkTask *insertOChunkTask = new ChunkTask(opInsert, updateSubjectID, object, objType, scanType, taskPackage, indexForTTForInsert);
		taskEnQueue(insertOChunkTask, xChunkQueue[ORDERBYO][chunkID]);
		indexForTTForInsert->wait();
	}
	delete buffer;
...
void PartitionMaster::executeChunkTaskUpdate(ChunkTask *chunkTask, const ID chunkID, const uchar* startPtr, const bool soType) {
	ID subjectID = chunkTask->Triple.subjectID;
	double object = chunkTask->Triple.object;
	char objType = chunkTask->Triple.objType;
...
void PartitionMaster::findObjectByPredicateAndSubject(const ID subjectID, MidResultBuffer *midResultBuffer, const double min, const double max, const uchar* startPtr) {
#ifdef MYDEBUG
	cout << __FUNCTION__ << " partitionID: " << partitionID << endl;
#endif
	...
	limit = chunkBegin + metaData->usedSpace;
	while (reader < limit) {
		reader = partitionChunkManager[ORDERBYS]->readXY(reader, tmpSubjectID, tmpObject, tmpObjType);
...
void PartitionMaster::findObjectByPredicateAndSubject(const ID subjectID, MidResultBuffer *midResultBuffer, const uchar *startPtr) {
	ID tmpSubjectID;
	double tmpObject;
	char tmpObjType;
	const uchar *reader, *limit, *chunkBegin = startPtr;
...
void PartitionMaster::findSubjectIDByPredicateAndObject(const double object, const char objType, MidResultBuffer *midResultBuffer, const ID minID, const ID maxID, const uchar* startPtr) {
#ifdef MYDEBUG
	cout << __FUNCTION__ << " partitionID: " << partitionID << endl;
#endif
	if (minID == 0 && maxID == UINT_MAX) {
		findSubjectIDByPredicateAndObject(object, objType, midResultBuffer, startPtr);
		return;
	}
	ID tmpSubjectID;
	double tmpObject;
	...
	limit = chunkBegin + metaData->usedSpace;
	while (reader < limit) {
		reader = partitionChunkManager[ORDERBYO]->readXY(reader, tmpSubjectID, tmpObject, tmpObjType);
...
void PartitionMaster::findSubjectIDByPredicateAndObject(const double object, const char objType, MidResultBuffer *midResultBuffer, const uchar *startPtr) {
	ID tmpSubjectID;
	double tmpObject;
	char tmpObjType;
	const uchar *reader, *limit, *chunkBegin = startPtr;
